...
TEST_PASSWORD = os.environ.get("TEST_PASSWORD", "pass")  # noqa: S2068
...
class TestExamModel(TestCase):
    ...
    def test_exam_ordering(self):
        Exam.objects.create(
            name="GATE ME", category="Eng", total_marks=100, exam_date=date.today()
        )
        Exam.objects.create(
            name="GATE CS", category="Eng", total_marks=100, exam_date=date.today()
        )
...
class TestSubjectModel(TestCase):
    def test_create_subject(self):
        ...
        subject = Subject.objects.create(exam=exam, name="Data Structures", order=1)
    ...
    def test_subject_string_representation(self):
        ...
        subject = Subject.objects.create(exam=exam, name="Algorithms")
...
class TestTopicModel(TestCase):
    def test_create_topic(self):
        ...
        topic = Topic.objects.create(
            name="Graphs", subject=subject, weightage=5.0, pyq_count=10, is_core=True
        )
    ...
    def test_topic_hierarchy(self):
        ...
        subject = Subject.objects.create(exam=exam, name="Data Structures")
        parent_topic = Topic.objects.create(name="Data Structures", subject=subject)
        child_topic = Topic.objects.create(
            name="Trees", subject=subject, parent=parent_topic
        )
...
class TestSubtopicModel(TestCase):
    def test_create_subtopic(self):
        ...
        subtopic = Subtopic.objects.create(topic=topic, name="DFS", order=1)
    ...
    def test_subtopic_string_representation(self):
        ...
        subject = Subject.objects.create(exam=exam, name="Data Structures")
        topic = Topic.objects.create(name="Graphs", subject=subject)
        subtopic = Subtopic.objects.create(topic=topic, name="BFS")
...
class TestRoadmapModel(TestCase):
    ...
    def test_roadmap_without_exam(self):
        ...
        roadmap = Roadmap.objects.create(
            user=user, target_date=date.today() + timedelta(days=90)
        )
...
class TestRoadmapGenerationJobModel(TestCase):
    def test_create_job(self):
        ...
        job = RoadmapGenerationJob.objects.create(user=user, status="pending")
    ...
    def test_job_string_representation(self):
        ...
        job = RoadmapGenerationJob.objects.create(user=user, status="processing")
...
class TestUserPYQAttemptModel(TestCase):
    def test_create_pyq_attempt(self):
        ...
        attempt = UserPYQAttempt.objects.create(
            user=user, pyq=pyq, answer={"selected": "A"}, is_correct=True, time_taken=30
        )
...
class TestDeterministicRoadmapGenerateSerializer(TestCase):
    def test_valid_data(self):
        ...
        serializer = DeterministicRoadmapGenerateSerializer(
            data={
                "exam_id": exam.id,
...
class TestRoadmapListView(TestCase):
    ...
    def test_list_roadmaps_only_returns_user_roadmaps(self):
        ...
        Roadmap.objects.create(
            user=user1, exam=exam, target_date=date.today() + timedelta(days=90)
        )
        Roadmap.objects.create(
            user=user2, exam=exam, target_date=date.today() + timedelta(days=90)
        )
        client = APIClient()
...
class TestRoadmapDetailView(TestCase):
    ...
    def test_patch_roadmap(self):
        ...
        response = client.patch(
            f"/api/roadmap/{roadmap.id}/", {"description": "New desc"}, format="json"
        )
        self.assertEqual(response.status_code, 200)
...
class TestRoadmapJobStatusView(TestCase):
    ...
    def test_get_job_status_unauthorized(self):
        ...
        job = RoadmapGenerationJob.objects.create(user=user1, status="pending")
        client = APIClient()
        client.force_authenticate(user=user2)
        response = client.get(f"/api/roadmap/job/{job.id}/")
        self.assertEqual(response.status_code, 404)
