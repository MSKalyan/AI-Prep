...
function handleTopicKeyDown(handler: () => void) {
...
export default function WeekPlanner({ roadmapId, week, studyMode = false, onSelectTopic }: Readonly<Props>) {
  ...
  const currentDay = (() => {
    for (const day of days) {
      ...
      const allCompleted = dayItems.every((t: WeekTopic) => t.completed || t.is_completed);
  ...
  async function handleToggle(id: number) {
    const response: { data?: WeekTopic[] } | undefined = queryClient.getQueryData([
      'week-topics',
      roadmapId,
      week,
    ]);
    const topicsList = response?.data || [];
    const topic = topicsList.find((t: WeekTopic) => t.id === id);
    const wasCompleted = topic?.completed ?? topic?.is_completed;

    await toggleTopic(id);

    queryClient.setQueryData<WeekPlanResponse>(['week-topics', roadmapId, week], (old) => {
      if (!old) return old;
      return {
        ...old,
        data: old.data.map((t: WeekTopic) =>
          t.id === id ? { ...t, completed: !wasCompleted, is_completed: !wasCompleted } : t
        ),
      };
    });
  }
  ...
  const handleWeekToggle = () => {
    if (!studyMode) setOpen(!open);
  };

  const handleWeekKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' || e.key === ' ') {
      if (!studyMode) setOpen(!open);
    }
  };
  ...
  return (
    <div className="border rounded-xl bg-white shadow-sm overflow-hidden mb-4">
      <button
        type="button"
        className={`flex justify-between items-center p-5 cursor-pointer w-full text-left ${open ? 'bg-gray-50' : 'hover:bg-gray-50'}`}
      ...
      {open && (
        <div className="p-4 space-y-6 border-t bg-gray-50/30">
          {days.map((day) => {
            ...
            const completedCount = dayTopics.filter(
              (t: WeekTopic) => t.completed || t.is_completed
            ).length;
            ...
            return (
              <div key={day} className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-gray-400">{dayProgress}% Done</span>
                </div>

                {day === currentDay && revision.length > 0 && (
                  <button
                    type="button"
                    className="p-3 bg-red-50 border border-red-200 rounded-lg cursor-pointer hover:bg-red-100 transition w-full text-left"
                    onClick={() => {
                      const selected = [...revision].sort(
                        (a, b) => (b.priority ?? 0) - (a.priority ?? 0)
                      ).at(0);
                      if (!selected?.roadmap_topic_id) return;
                      router.push(
                  ...
                  {dayTopics.map((t: WeekTopic) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-4 w-full text-left  border rounded-lg p-3 cursor-pointer transition ${
                        ...
                        {!studyMode && (
                          <button
                            className={`px-3 py-1 text-xs font-bold rounded ${
                              isMock ? 'bg-purple-50 text-purple-600' : 'bg-blue-50 text-blue-600'
                            }`}
                            onClick={(e) => {
                              e.stopPropagation();
